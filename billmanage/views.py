...
def reportbill(request):
    ...
    if request.method == "POST":
        ...
        if status == "" and remark == "":
            # print(fromdate)
            itbill = bill.objects.filter(
                date__gte=fromdate, date__lte=todate).order_by("date")

        elif str(request.POST.get("remark")) == "" and status != "":
            itbill = bill.objects.filter(
                date__gte=fromdate, date__lte=todate, status=status).order_by("date")

        elif remark != "" and status == "":
            itbill = bill.objects.filter(
                date__gte=fromdate, date__lte=todate, remark__startswith=remark).order_by("date")
            print("hello")

        else:
            itbill = bill.objects.filter(
                date__gte=fromdate, date__lte=todate, remark__startswith=remark, status=status).order_by("date")

        r.fromQueryset(itbill, {"code": "id", "bill": "billno", "date": {
                       "name": "date", "format": "%d-%m-%Y"}, "remark": "remark", "amount": "amount", "status": "status"})
...
def print_view(request):
    ...
    if request.method == "POST":
        ...
        todate = datetime.strptime(todt, '%Y-%m-%d')
        # print(remark)
        if str(request.POST.get("remark")) == "" and status == "":
            # print(fromdate)
            itbill = bill.objects.filter(
                date__gte=fromdate, date__lte=todate).order_by("date")
            ...
            itbill = bill.objects.filter(
                date__gte=fromdate, date__lte=todate, status="Paid").order_by("date")
        elif status == "Unpaid":
            itbill = bill.objects.filter(
                date__gte=fromdate, date__lte=todate, status="Unpaid").order_by("date")

        else:
            itbill = bill.objects.filter(
                date__gte=fromdate, date__lte=todate, remark__startswith=remark, status=status).order_by("date")
        r.fromQueryset(itbill, {"date": {"name": "date", "format": "%d-%m-%Y"},
                       "remark": "remark", "amount": "amount", "status": "status"})
        resp.success("data ok")
        resp.setExtra(r.data)
        print(itbill)
        return HttpResponse(resp.getJson())
